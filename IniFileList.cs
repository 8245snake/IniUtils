...
namespace IniUtils
{
    public class IniFileList : IDictionary<string, IniFile>, ICollection<IniFile>, IEnumerable<IniFile>, ICloneable
    {
        ...
        /// <summary>
        /// コレクションを入れて初期化するコンストラクタ
        /// </summary>
        /// <param name="list">初期化に使用するコンストラクタ</param>
        ...
        /// 設定をすべて書き出す
        /// </summary>
        /// <param name="directory">書き出すフォルダパス（既にあれば上書きされる）</param>
        ...
        /// <remarks>augendとaddendの和集合を返す。両方に存在するキーの場合はaugendの値を優先する。</remarks>
        ...
        /// <remarks>minuendにしかないキーと両方にあり値が異なるキーの集合を返す。
        /// 両方に存在するキーの場合はminuendの値を優先する。</remarks>
        ...
        /// <remarks>dividendのみに存在する要素を返す</remarks>
